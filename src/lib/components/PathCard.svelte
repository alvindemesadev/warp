<script lang="ts">
  ...
  let {
    ...
    isScanning, isScanningDest, dragTarget, isDragging,
    onBrowseSource, onBrowseDest, onClearSource, onClearDest, onSwap
  }: {
    sourcePath: string; destPath: string;
    sourceInfo: PathInfo | null; destInfo: PathInfo | null;
    isScanning: boolean; isScanningDest: boolean;
    dragTarget: string | null; isDragging: boolean;
    onBrowseSource: () => void; onBrowseDest: () => void;
...
<div class="card" class:card--drag-active={isDragging && sourcePath && destPath}>
  ...
  <div role="button" tabindex="0" class="row row--source" class:row--drag={dragTarget==='source'} onclick={onBrowseSource} onkeydown={(e) => { if (e.key==='Enter'||e.key===' ') { e.preventDefault(); onBrowseSource(); } }}>
    ...
    <div class="text">
      ...
      {#if sourcePath}
        <p class="path" class:path--error={sourceInfo?.isFile} title={sourcePath}>{basename(sourcePath)}</p>
        <p class="meta">
  ...
  <!-- Swap -->
  <div class="swap-wrap">
    <button onclick={onSwap} title="Swap source and destination" disabled={!sourcePath && !destPath} class="swap" class:swap--enabled={!!(sourcePath||destPath)}>
      <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M3 1.5v7M1.5 7L3 8.5 4.5 7M7 8.5v-7M5.5 3L7 1.5 8.5 3" stroke="rgba(255,255,255,0.35)" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </button>
  </div>
  ...
  <div role="button" tabindex="0" class="row row--dest" class:row--drag={dragTarget==='dest'} onclick={onBrowseDest} onkeydown={(e) => { if (e.key==='Enter'||e.key===' ') { e.preventDefault(); onBrowseDest(); } }}>
    ...
    <div class="text">
      ...
      {#if destPath}
        <p class="path" class:path--error={destInfo?.isFile} title={destPath}>{basename(destPath)}</p>
        <p class="meta">
...
<style>
  ...
  .icon { width: 34px; height: 34px; border-radius: 9px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: rgba(255,255,255,0.06); }
  .icon--source { background: rgba(10,132,255,0.18); }
  .icon--dest { background: rgba(48,209,88,0.15); }
  .icon--error { background: rgba(255,69,58,0.15); }
  .text { flex: 1; min-width: 0; }
  .label { font-size: 9px; font-weight: 700; color: var(--text-tertiary); letter-spacing: 0.07em; text-transform: uppercase; margin: 0 0 2px; }
  .path { font-size: 13px; font-weight: 500; color: var(--text-primary); margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .path--error { color: var(--red); }
  .meta { font-size: 10px; color: var(--text-tertiary); margin: 1px 0 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .placeholder { font-size: 13px; color: var(--text-tertiary); margin: 0; }
  .link { background: none; border: none; color: var(--accent); font-size: 13px; font-family: var(--font-sf); cursor: pointer; padding: 0; text-decoration: underline; text-underline-offset: 2px; }
  .hint { font-size: 9px; color: var(--orange); margin: 2px 0 0; }
  .pulse { animation: pulse-soft 1.8s ease-in-out infinite; }
  .error { color: var(--red); }
  .warn { color: var(--orange); }
  .clear { width: 18px; height: 18px; border-radius: 50%; background: rgba(255,255,255,0.08); border: none; padding: 0; display: flex; align-items: center; justify-content: center; flex-shrink: 0; cursor: pointer; opacity: 0.5; transition: opacity 0.15s; }
  .clear:hover { opacity: 1; }
  .swap-wrap { height: 0; position: relative; display: flex; justify-content: center; }
  .swap { position: absolute; top: -11px; z-index: 10; width: 22px; height: 22px; border-radius: 50%; background: #111; border: 1px solid var(--glass-border); display: flex; align-items: center; justify-content: center; cursor: default; transition: background 0.15s, border-color 0.15s; }
  .swap--enabled { cursor: pointer; }
  .swap--enabled:hover { background: #222; border-color: rgba(255,255,255,0.2); }
</style>
