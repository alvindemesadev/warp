<script lang="ts">
  ...
  let { summary, mode, sourcePath, destPath, errorLogs = [], onReset }: { summary: WarpSummary; mode: string; sourcePath: string; destPath: string; errorLogs: string[]; onReset: () => void } = $props();
</script>

<div class="wrap animate-fade-up">
  <div class="card">
    <div class="head">
      <div class="icon" class:icon--cancel={summary.cancelled} class:icon--error={summary.failed>0} class:icon--ok={!summary.cancelled && summary.failed===0}>
...
<style>
  ...
  .paths { background: rgba(255,255,255,0.03); border: 1px solid var(--glass-border); border-radius: 12px; padding: 11px 14px; display: flex; flex-direction: column; gap: 5px; }
  .path-row { display: flex; align-items: center; gap: 8px; }
  .path-label { font-size: 9px; font-weight: 700; color: var(--text-tertiary); letter-spacing: 0.06em; text-transform: uppercase; width: 26px; flex-shrink: 0; }
  .path-val { font-size: 11px; color: var(--text-secondary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .logs { background: rgba(255,69,58,0.05); border: 1px solid rgba(255,69,58,0.15); border-radius: 12px; padding: 11px 14px; }
  .logs-title { font-size: 9px; font-weight: 700; color: var(--red); letter-spacing: 0.06em; text-transform: uppercase; margin: 0 0 7px; }
  .logs-list { max-height: 72px; overflow-y: auto; display: flex; flex-direction: column; gap: 2px; }
  .log { font-size: 10px; font-family: monospace; color: rgba(255,69,58,0.65); margin: 0; }
  .btn { width: 100%; padding: 12px; border-radius: 14px; border: none; font-size: 14px; font-weight: 600; letter-spacing: -0.01em; background: var(--accent); color: white; box-shadow: 0 2px 20px rgba(10,132,255,0.28); cursor: pointer; transition: all 0.15s; outline: none; }
