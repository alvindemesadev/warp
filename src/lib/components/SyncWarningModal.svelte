<script lang="ts">
  ...
  let { destPath, onCancel, onConfirm }: { destPath: string; onCancel: () => void; onConfirm: () => void } = $props();
</script>

<div class="overlay" role="dialog" aria-modal="true" aria-label="Sync will delete files" tabindex="-1">
  ...
  <div class="modal">
    ...
    <p class="modal-body">
      Sync mirrors the source exactly. Any file in <strong class="strong">{basename(destPath)}</strong> that doesn't exist in the source will be <strong class="strong strong--red">permanently deleted</strong>.
    </p>
...
<style>
  ...
  .icon { width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  .icon--red { background: rgba(255,69,58,0.15); }
  .modal-title { font-size: 14px; font-weight: 600; color: var(--text-primary); margin: 0; }
  .modal-sub { font-size: 11px; color: var(--text-tertiary); margin: 3px 0 0; }
  .modal-body { font-size: 12px; color: var(--text-secondary); line-height: 1.5; margin: 0 0 18px; }
  ...
  .btn--danger { border: none; background: var(--red); color: white; font-weight: 600; }
